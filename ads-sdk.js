/**
 * Linotype Ads SDK - Production Ready
 * Version: 1.0.0
 * 
 * Features:
...
(function(global) {
  ...
  const utils = {
    log: function(...args) {
      if (config.debug || (global.SORON_ADS_DEBUG === true)) {
        console.log('[LinotypeAds]', ...args);
      }
    },
    error: function(...args) {
      console.error('[LinotypeAds]', ...args);
    },
    generateUserId: function() {
      // Generate anonymous user ID if not provided
      const stored = localStorage.getItem('linotype_user_id');
      if (stored) return stored;
      
      const newId = 'anon_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
      localStorage.setItem('linotype_user_id', newId);
      return newId;
    },
  ...
  class LinotypeAdsSDK {
    constructor(options = {}) {
      ...
      this.timeout = options.timeout || config.defaultTimeout; // Custom timeout per instance
      this.callbacks = {};
    ...
    async getAd(input, options = {}) {
      ...
      for (let attempt = 0; attempt <= config.maxRetries; attempt++) {
        try {
          ...
          // Handle fallback response from error handling
          if (response.fallback) {
            utils.log('Using fallback response due to API error');
      ...
      // If we get here, all attempts failed but we have a fallback
      utils.log('All retry attempts failed, returning null');
      return null;
    }
    ...
    async getAdForAgentResponse(context, options = {}) {
      ...
      try {
        ...
        if (ad) {
          ...
          // Return empty string if no ad, so it doesn't break the main flow
          return '';
        }
      } finally {
     ...
     */
    async getAdForUserQuery(context, options = {}) {
      const originalMode = this.mode;
    ...
    async _makeRequest(endpoint, payload) {
      ...
      try {
        ...
        utils.error('Ad request failed:', error.message);
        
        // Return a safe fallback response that won't break the main flow
        return {
          ads: [],
          error: error.message,
          fallback: true
    ...
    _attachClickHandlers(container, ad) {
      const links = container.querySelectorAll('a[data-linotype-click]');
    ...
    _getDefaultTemplate() {
      return (ad) => {
        ...
        const safeContent = this._escapeHtml(ad.content);
        const safeCta = this._escapeHtml(ad.cta);
        const safeUrl = this._escapeHtml(linkUrl);
    
        let contentWithLink = safeContent;
    
        if (hasLink) {
          const lastIndex = safeContent.lastIndexOf(safeCta);
    
          if (lastIndex !== -1) {
            // Replace last CTA with a linked CTA
            contentWithLink =
              safeContent.substring(0, lastIndex) +
              `<a href="${safeUrl}" target="_blank" rel="noopener noreferrer" data-linotype-click="true" style="color: #007bff; text-decoration: none;">${safeCta}</a>` +
              safeContent.substring(lastIndex + safeCta.length);
          } else {
            // CTA not found → append fallback "Learn more here!"
            contentWithLink =
              safeContent +
              ` <a href="${safeUrl}" target="_blank" rel="noopener noreferrer" data-linotype-click="true" style="color: #007bff; text-decoration: none;">Learn more here!</a>`;
          }
        }
    
        return `${contentWithLink} (sponsored)`;
      };
    }
  ...
  const simpleAPI = {
    init: function(apiKey, options = {}) {
      this.instance = new LinotypeAdsSDK({ apiKey, ...options });
      return this.instance;
    },
    
    getAd: async function(input, container, options = {}) {
      if (!this.instance) {
        throw new Error('Call LinotypeAds.init(apiKey) first');
      }
  ...
  // Export
  global.LinotypeAdsSDK = LinotypeAdsSDK;
  global.LinotypeAds = simpleAPI;
  
  // AMD/CommonJS/ES6 module support
  if (typeof define === 'function' && define.amd) {
    define([], function() { return LinotypeAdsSDK; });
  } else if (typeof module === 'object' && module.exports) {
    module.exports = LinotypeAdsSDK;
  }
